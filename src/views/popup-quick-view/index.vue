...
const islandCtx = createContext({ pluginId: manifest.id, viewName: 'popup-quick-view' })

onMounted(async () => {
  const noteId = await islandCtx.plugin.store.get('editNoteId')
  const storedNotes = await islandCtx.plugin.store.get('notes') || []
